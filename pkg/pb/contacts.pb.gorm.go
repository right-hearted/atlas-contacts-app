...
type NetworkORM struct {
	...
	Cidr      int32
	Comment   string
	Fixed     []*IPv4ORM `gorm:"foreignkey:NetworkId;association_foreignkey:Id"`
	Id        int64      `gorm:"type:serial;primary_key"`
}
...
// TableName overrides the default tablename generated by GORM
func (IPv4ORM) TableName() string {
	return "ipv4"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *IPv4) ToORM(ctx context.Context) (IPv4ORM, error) {
	to := IPv4ORM{}
	var err error
	if prehook, ok := interface{}(m).(IPv4WithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	to.AccountID = accountID
	if posthook, ok := interface{}(m).(IPv4WithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *IPv4ORM) ToPB(ctx context.Context) (IPv4, error) {
	to := IPv4{}
	var err error
	if prehook, ok := interface{}(m).(IPv4WithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	to.Addr = m.Addr
	if posthook, ok := interface{}(m).(IPv4WithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type IPv4 the arg will be the target, the caller the one being converted from

// IPv4BeforeToORM called before default ToORM code
type IPv4WithBeforeToORM interface {
	BeforeToORM(context.Context, *IPv4ORM) error
}

// IPv4AfterToORM called after default ToORM code
type IPv4WithAfterToORM interface {
	AfterToORM(context.Context, *IPv4ORM) error
}

// IPv4BeforeToPB called before default ToPB code
type IPv4WithBeforeToPB interface {
	BeforeToPB(context.Context, *IPv4) error
}

// IPv4AfterToPB called after default ToPB code
type IPv4WithAfterToPB interface {
	AfterToPB(context.Context, *IPv4) error
}
...
func DefaultStrictUpdateNetwork(ctx context.Context, in *Network, db *gorm1.DB) (*Network, error) {
	...
	*filterFixed.NetworkId = ormObj.Id
	...
	if err = db.Where(filterFixed).Delete(IPv4ORM{}).Error; err != nil {
		return nil, err
	}
...
// DefaultCreateIPv4 executes a basic gorm create call
func DefaultCreateIPv4(ctx context.Context, in *IPv4, db *gorm1.DB) (*IPv4, error) {
	if in == nil {
		return nil, errors.New("Nil argument to DefaultCreateIPv4")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
...
// DefaultListIPv4 executes a gorm list call
func DefaultListIPv4(ctx context.Context, db *gorm1.DB, req interface{}) ([]*IPv4, error) {
	ormResponse := []IPv4ORM{}
	f, s, p, fs, err := getCollectionOperators(req)
	if err != nil {
		return nil, err
	}
	db, err = gorm2.ApplyCollectionOperators(db, &IPv4ORM{}, f, s, p, fs)
	if err != nil {
		return nil, err
	}
	in := IPv4{}
...
func (m *NetworksDefaultServer) ListFixed(ctx context.Context, in *ListFixedRequest) (*ListFixedResponse, error) {
	...
	db := m.DB
	res, err := DefaultListIPv4(ctx, db, in)
	if err != nil {
		return nil, err
	}
